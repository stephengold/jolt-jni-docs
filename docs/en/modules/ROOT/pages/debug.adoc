...
such as "com.github.stephengold:jolt-jni-Windows64:6.0.0:DebugSp".
...
* yellow for a rigid body
  (without contact response) such as a sensor,
* magenta for a rigid body
  (with contact response) that's dynamic and active,
* dark gray for rigid body
  (with contact response) that's kinematic and active,
* light gray for a rigid body
  (with contact response) that's static or deactivated,
