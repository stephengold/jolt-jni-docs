...
use manifolds and contact points to implement contact response,
...
== Subshapes in query results

Query results typically report the ID of the body that was hit.
If the body has a compound shape,
the result should also include the 32-bit ID of the subshape that was hit.

Because compound shapes can contain other compound shapes, 
subshape IDs are assigned in a non-obvious fashion.
To access a subshape by its ID, use the `getLeafShape()` method.
For example:
...
if (lock.succeeded()) {
    ConstShape bodyShape = lock.getBody().getShape();
    int subShapeId = hit.getSubShapeId2();
    int[] remainder = {Jolt.cEmptySubShapeId};
    ConstShape subShape = bodyShape.getLeafShape(subShapeId, remainder);
    // do something with subShape
}
lock.releaseLock();
----
