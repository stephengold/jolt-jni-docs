...
Native coding conventions dictate that when a class allocates memory,
...
its memory can be freed in any of 3 ways:
...
{Project} automatically creates a reference to it.
...
* Explicitly closing a `JoltPhysicsObject` is potentially dangerous.
