...
{url-tutorial}/HelloRigidBody.java[HelloRigidBody] is a {SportAdj} app
...
When adding a static body to its physics system,
there's no reason to activate it:
...
{url-tutorial}/HelloStaticBody.java[HelloStaticBody] is a {SportAdj} app
...
----
Body body = /* ... */;
RVec3 location = body.getPosition();
Quat orientation = body.getRotation();
----
...
----
Body body = /* ... */;
RVec3 comLocation = body.getCenterOfMassPosition();
----
...
{url-tutorial}/HelloKinematics.java[HelloKinematics] is a {SportAdj} app
...
{url-tutorial}/HelloDamping.java[HelloDamping] is a {SportAdj} app
that demonstrates damping.
Things to notice while running the app:

. Initially, all 4 cubes have identical linear and angular velocities.
...
. The linear velocities of the bottom 2 decay quickly to zero
  due to strong linear damping.
. The angular velocities of the right 2 decay quickly to zero
...
{url-tutorial}/HelloDeactivation.java[HelloDeactivation] is a {SportAdj} app
...
{url-tutorial}/HelloCcd.java[HelloCcd] is a {SportAdj} app
