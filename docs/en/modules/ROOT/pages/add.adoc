...
For now, I suggest putting all bodies in object layer 0
and mapping object layer 0 to broadphase layer 0.
...
// Enable collisions between bodies in object layer 0:
ObjectLayerPairFilterTable ovoFilter
        = new ObjectLayerPairFilterTable(numObjLayers);
ovoFilter.enableCollision(0, 0);

// Map object layer 0 to broadphase layer 0:
