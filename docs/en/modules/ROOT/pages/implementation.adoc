...
for 14 different platforms:

* Windows (x86-64 with and without AVX2; also aarch64),
* Linux (x86-64 with and without FMA, armhf, and aarch64),
* macOS (x86-64 and arm64), and
* Android (armeabi-v7a, arm64-v8a, riscv64, x86, and x86_64).
...
Each rigid body refers to a _collision shape_
that describes the shape of its surface.
Each soft body refers to a shared-settings object
that decomposes it into vertices, edges, faces, and constraints.
...
Bodies are simulated in discrete steps of short duration,
typically 1/60 of a second.
Hair simulations run separately from body simulations,
typically at 360 iterations per second.
...
The number of worker threads for simulating bodies
is configured in a `JobSystem`
that's typically created during initialization.
Hair simulation typically runs on a GPU,
which is represented as a `ComputeSystem`.
