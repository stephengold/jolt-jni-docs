...
_{Project}_ is a {url-enwiki}/Physics_engine[3-D physics-simulation] library
for {url-enwiki}/List_of_JVM_languages[JVM languages]
such as {url-enwiki}/Java_(programming_language)[Java],
{url-enwiki}/Python_(programming_language)[Jython],
{url-enwiki}/Nashorn_(JavaScript_engine)[Nashorn],
{url-enwiki}/Kotlin[Kotlin],
{url-enwiki}/Apache_Groovy[Groovy],
{url-enwiki}/Scala_(programming_language)[Scala], and 
{url-enwiki}/Clojure[Clojure].
...
* simulate cloth, hair, rope, and deformable volumes,
...
For generating collision shapes, {Project} also incorporates source code from
...
To simulate hair, I recommend a computer
with a {url-enwiki}/Graphics_processing_unit[GPU] and a compatible API:
Metal (on macOS) or Vulkan (on Android/Linux/Windows).
...
LoongArch, PowerPC, or 32-bit Windows.
...
If you plan to contribute to {Project}, please read
https://github.com/stephengold/jolt-jni/blob/master/CONTRIBUTING.md[the contributor guide].
...
https://github.com/stephengold/jolt-jni/tree/master/src[the source code].
